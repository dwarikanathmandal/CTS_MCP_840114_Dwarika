...
import { getDocsByTaskId, deleteDoc, clearDocuments } from '../../../../store/actions/document';
...
const DocumentList = props => {
    ...
    const toggleModel = () => {
        ...
        if (taskId && taskId.length === 36) {
            dispatch(getDocsByTaskId(`${apiConfig.baseUrl}Documents/GetDocumentsByTaskId`, taskId));
        }
        else {
        ...
        <Container>
            <Row>
                <Col style={{ ...styles.textAlignRight, ...styles.addButtonCol }}>
                    <Button type='button' onClick={() => toggleModel()}>Add Document</Button>
                </Col>
            </Row>
            <Table>
                <thead>
                    <tr>
                        <th></th>
                        <th>Document Name</th>
                        <th style={{ ...styles.actionColumn, ...styles.textAlignRight }}>Action</th>
                    </tr>
                </thead>
                <tbody>
                    {
                                ...
                                return <tr key={doc.id}>
                                    ...
                                    <td>
                                        <span
                                            ...
                                            onClick={() => downloadDocument(doc.documentName, doc.id)}>
                                            {doc.documentName}
                                        </span>
                                    </td>
            ...
            <Modal isOpen={isRemoveModelOpen} toggle={toggleRemoveModel} className={''}>
                ...
                <ModalFooter>
                    ...
                    <Button color="primary" type='button' onClick={removeDocument}>Remove</Button>
                </ModalFooter>
            </Modal>
        </Container>
    )
